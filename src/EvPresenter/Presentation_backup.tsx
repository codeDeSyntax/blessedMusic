  ...
  return (
    ...
    >
      <div className="w-full h-full flex items-center justify-center p-6 sm:p-8 md:p-12">
        <div className="w-full  max-h-full overflow-scroll no-scrollbar text-center p-6 md:p-8 font-archivo rounded-xl shadow-xl text-[#9a674a] dark:text-white relative">
...
export const PresentationSlideshow: React.FC<{ onBack: () => void }> = ({
  ...
  useEffect(() => {
    // First priority: Use presentation's own background image if it exists
    if (currentPresentation?.backgroundImage) {
      setBackgroundImage(currentPresentation.backgroundImage);
    } else {
      // Second priority: Check if there's a saved background from global settings
      const savedBg = localStorage.getItem("selectedBg");

      if (savedBg) {
        // Use the saved global background
  ...
  }, [currentSlide, isMessagePointsAutoScrollPaused, currentPresentation]);
      ...
      if (currentPresentation.type === "sermon") {
                ...
                <div className="w-2/3 h-full  relative flex items-center justify-center overflow-hidden bg-white">
                  ...
                  <div className="relative  mx-16 z-10">
                    ...
                    <motion.div
                      ...
                      className="relative border-4 border-white/20 p-12 border-dashed max-h-[70vh] overflow-hidden"
                    >
        ...
        if ((currentPresentation as any).mainMessagePoints?.length > 0) {
          newSlides.push(
            ...
            <div className="w-full h-[90vh] flex justify-center items-center rounded-3xl m-auto">
              {/* Left side - Background image with centered title (1/3 width) */}

              {/* Right side - Main message points with blurred background (2/3 width) */}
              <div className="w-full h-[80%] relative flex flex-col p-12 overflow-hidden rouded-3xl">
                {/* Blurred background image */}
                <div
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="flex-1 overflow-hidden relative">
                    ...
                    <div
                      ...
                      onMouseEnter={() =>
                        setIsMessagePointsAutoScrollPaused(true)
                      }
                      onMouseLeave={() =>
                        setIsMessagePointsAutoScrollPaused(false)
                      }
                    >
      ...
      {slideView === "grid" ? (
        ...
        >
          {slides.map((slide, index) => (
            ...
            >
              <div className="absolute inset-0 transform scale-[0.25] origin-center">
