...
import { SlideTemplates, TemplateType, SlideContentData } from "./templates/SlideTemplates";
import { PresentationThemes, ThemeType, getThemeByName, getAllThemeNames } from "./templates/ThemeColors";

export const EnhancedPresentationSlideshow: React.FC<{ onBack: () => void }> = ({
  onBack,
}) => {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<TemplateType>(
    () => (localStorage.getItem("presentationTemplate") as TemplateType) || "minimal"
  );
  ...
  const generateSlides = useCallback(() => {
    ...
    newSlides.push({
      ...
      subtitle: `${currentPresentation.preacher} • ${new Date(currentPresentation.date).toLocaleDateString()}`,
      preacher: currentPresentation.preacher,
      date: new Date(currentPresentation.date).toLocaleDateString(),
    });

    // Scripture slides
    if (currentPresentation.scriptures && currentPresentation.scriptures.length > 0) {
      currentPresentation.scriptures.forEach((scripture) => {
        newSlides.push({
          title: "Scripture",
          scripture: scripture.text,
          reference: "Holy Bible", // You might want to add reference to Scripture type
        });
      });
    }
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-100">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-700 mb-4">No Presentation Loaded</h2>
          <button
            onClick={onBack}
          ...
          <div className="flex items-center space-x-4 bg-white/90 backdrop-blur-md rounded-full px-6 py-3 shadow-lg">
            ...
            <button
              ...
              className={`p-2 rounded-full transition-colors ${
                isAutoPlaying ? "bg-blue-100 text-blue-600" : "hover:bg-gray-200"
              }`}
            >
